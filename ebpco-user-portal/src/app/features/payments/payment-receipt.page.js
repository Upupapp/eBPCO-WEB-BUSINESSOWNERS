 ...
 */
let PaymentReceiptPage = class PaymentReceiptPage {
    ...
    isOfficial = computed(() => !!this.payment()?.orNumber);
    header = computed(() => {
        const a = this.app();
        const reviewingOffice = a && a.permitType !== 'Business Permit' ? requirementsFor(a.permitType).reviewingOffice : 'Office of the Building Official (OBO)';
    ...
    gateCleared = computed(() => this.watermarkText() === null && !!this.payment());
    print() {
        window.print();
    }
};
PaymentReceiptPage = __decorate([
    Component({
