...
let VerifyPermitPage = class VerifyPermitPage {
    ...
    businessLabel = computed(() => this.business()?.name || this.app()?.businessName || 'Not provided');
    documentTitle = computed(() => {
        const a = this.app();
        if (!a || a.permitType === 'Business Permit')
            return a?.permitType ?? '';
    ...
    status = computed(() => {
        ...
        if (p.provenance !== 'issued')
            return 'Unverified';
        if (!isPermitStanding(p.standing))
            return 'Unverified';
        ...
        if (p.expiryDateValue && p.expiryDateValue.getTime() < Date.now())
            return 'Expired';
        return p.standing;
    });
    badgeClass = computed(() => {
