...
import { actionNeedsExistingPermit, actionReferenceIsComplete, } from '../../core/domain/application.model';
...
describe('A Renewal or Amendment names the permit it acts on', () => {
    ...
    it('the store REFUSES a Renewal that names no permit', () => {
        expect(() => store.createDraft({ ...base, applicationAction: 'Renewal', relatedPermitNumber: null })).toThrow(/must name the permit/i);
    });
    it('the store REFUSES an Amendment that names no permit', () => {
        expect(() => store.createDraft({ ...base, applicationAction: 'Amendment', relatedPermitNumber: null })).toThrow(/must name the permit/i);
    });
    it('the store REFUSES a New application that names one — a relationship never claimed', () => {
        expect(() => store.createDraft({ ...base, applicationAction: 'New', relatedPermitNumber: 'BP-2025-00001' })).toThrow(/must name the permit/i);
    });
    ...
    it('only permits ISSUED to the signed-in citizen are offered', () => {
        ...
        const known = new Set(store.myApplications().map((a) => a.permitNumber).filter((n) => n !== null));
        for (const p of offered)
            expect(known.has(p.permitNumber)).toBe(true);
    });
