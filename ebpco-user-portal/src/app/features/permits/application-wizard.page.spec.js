...
describe('Attachments carry the file, not just its name', () => {
    ...
    it('carries the file the citizen actually picked, from onFileSelected to the store', async () => {
        ...
        const event = { target: { files: [chosen] } };
        ...
        if (slot.kind !== 'upload')
            throw new Error('a chosen file must be an upload slot');
        ...
        appStore.attachDocument(app.id, requirement.id, requirement.label, slot.file, 'pdf');
