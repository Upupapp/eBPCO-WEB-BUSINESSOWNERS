...
describe('Reusing a document already on file', () => {
    ...
    const realFile = () => new File([new Uint8Array([0x25, 0x50, 0x44, 0x46, 9, 9])], 'survey-plan.pdf', { type: 'application/pdf' });
    ...
    it('a seeded row with no file is excluded even though it is in the library', () => {
        ...
        for (const d of withoutBytes)
            expect(offeredIds.has(d.id)).toBe(false);
    });
    it('reusing attaches the ACTUAL File, not just its name', async () => {
        ...
        if (attached.kind !== 'upload')
            throw new Error('expected an upload slot');
    ...
    it('a file-less document attaches NOTHING rather than a name', () => {
        const hollow = { id: 'x', ownerId: 'u', fileName: 'ghost.pdf', fileType: 'pdf',
            category: 'supportingDocument', uploadedAt: '2026-01-01',
