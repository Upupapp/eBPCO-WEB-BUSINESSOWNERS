...
describe('ApplicationDocuments — the office speaking to the citizen', () => {
    ...
    it('renders the recorded sample: one unreviewed, one rejected WITH its reason', () => {
        const text = render(RECORDED).nativeElement.textContent ?? '';
    ...
    it('never gives an unreviewed document the green a citizen reads as done', () => {
        const el = render([doc({ reviewStatus: null })]).nativeElement;
    ...
    it('shows the rejection and its replacement together', () => {
        ...
        const text = (render([rejected, replacement]).nativeElement.textContent ?? '');
        // What I sent instead…
    ...
    it('keeps the virus scanner off the officer’s axis', () => {
        const quarantined = render([doc({ quarantined: true })]).nativeElement.textContent ?? '';
        ...
        const rejected = render([doc({ reviewStatus: 'Rejected' })]).nativeElement.textContent ?? '';
    ...
    it('a document with plenty of validity left does not nag', () => {
        ...
        const text = render([doc({ expiresOn: future })]).nativeElement.textContent ?? '';
    ...
    it('says nothing about validity when the office set no expiry', () => {
        const text = render([doc({ expiresOn: null })]).nativeElement.textContent ?? '';
    ...
    it('offers "replace" only where the office asked, never where the server would 409', () => {
        ...
        const askable = render([doc({ reviewStatus: 'Rejected' })]).nativeElement;
        ...
        for (const d of [doc({ reviewStatus: 'Accepted' }),
            ...
            expect(render([d]).nativeElement.querySelector('[data-action="replace"]')).toBeNull();
        }
    });
});
