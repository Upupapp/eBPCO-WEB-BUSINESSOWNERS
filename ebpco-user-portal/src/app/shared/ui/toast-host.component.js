...
let ToastHostComponent = class ToastHostComponent {
    toast = inject(ToastService);
    polite = computed(() => this.toast.toasts().filter((t) => t.kind !== 'error'));
    errors = computed(() => this.toast.toasts().filter((t) => t.kind === 'error'));
};
ToastHostComponent = __decorate([
    Component({
