...
function doc(id, label, required, description) {
...
const COMMON_DOCS_NO_LOCATIONAL = COMMON_DOCS.filter((d) => d.id !== 'locational');
const PENDING_NOTE = "Documentary requirements follow the generic national-law/reference format (PD 1096 for building-related permits, RA 9514 for fire-safety permits); the Municipality of Castilla's own confirmed checklist for this specific permit type is still pending verification with the OBO — confirm before production launch.";
...
export function requirementsFor(permitType) {
...
export const GENERIC_APPLICATION_DOCUMENTS = [
