...
const file = (name, bytes, lastModified = 1) => new File([new Uint8Array(bytes)], name, { type: 'application/pdf', lastModified });
...
describe('DocumentResubmission — what the citizen is told', () => {
    ...
    async function sendAndFail(body, status) {
        let err;
        svc.resubmit(APP, DOC, 'Lot plan', file('a.pdf', [1, 2, 3])).subscribe({ error: (e) => (err = e) });
        (await waitForRequest()).flush(body, { status, statusText: 'x' });
        return err;
    }
    it('passes a 409 detail through rather than saying "conflict"', async () => {
        ...
        const err = await sendAndFail({ title: 'Conflict', status: 409, detail: 'This document has already been accepted by an officer.' }, 409);
    ...
    it('returns removedMetadata, which the citizen is entitled to know about', async () => {
        ...
        (await waitForRequest()).flush({ ...CONTRACT_SAMPLES['applicant.applications.resubmitDocument'].body, removedMetadata: ['GPS', 'EXIF'] }, { status: 201, statusText: 'Created' });
