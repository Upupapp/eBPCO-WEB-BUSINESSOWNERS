...
import { defaultNotificationPreferences, unverifiedContact, } from '../domain/user.model';
...
let AuthService = class AuthService {
     ...
     */
    _applicantTypeOverride = signal(null);
    _preferences = signal(defaultNotificationPreferences());
     ...
     */
    isAuthenticated = computed(() => this._profile() !== null);
    ...
    async register(personal, contact, security) {
        try {
     ...
     */
    async restore() {
        if (!this.tokens.hasSession() || this._profile() !== null)
            return;
        try {
 ...
 */
function meResponseToAccount(me) {
    return {
        id: me.id,
