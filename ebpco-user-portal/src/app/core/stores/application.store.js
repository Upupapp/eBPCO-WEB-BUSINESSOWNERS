...
import { actionReferenceIsComplete } from '../domain/application.model';
import { LIFECYCLE_SEQUENCE, NEXT_STEP_TEXT, applicantStatusOf, } from '../domain/status.model';
import { ILLUSTRATIVE_FEE_BASIS } from '../domain/assessment.model';
 ...
 */
function fromServerSummary(row, applicantId) {
    return {
        id: row.id,
        ...
        applicationAction: row.applicationAction,
...
function describeApplicationError(error) {
    if (error instanceof ApiError)
        return error.citizenMessage;
    return 'We could not reach the Municipality’s system. Check your connection and try again.';
}
function prefixFor(permitType) {
    if (permitType === 'Business Permit')
        return 'E-BPCO';
    if (permitType.startsWith('Building Permit'))
        return 'BP';
    if (permitType === 'Zoning / Locational Clearance')
        return 'ZLC';
    if (permitType === 'FSEC for Building Permit (BFP)')
        return 'FSEC';
    if (permitType === 'FSIC for Occupancy Permit (BFP)')
        return 'FSIC';
    if (permitType === 'Certificate of Occupancy')
        return 'COO';
...
let ApplicationStore = class ApplicationStore {
    ...
    notifications;
    applications = signal([]);
    documentsByApp = signal({});
    assessmentsByApp = signal({});
    paymentsByApp = signal({});
    timelineByApp = signal({});
    permitsByApp = signal({});
    ...
    seed() {
        ...
        this.documentsByApp.set({
            ...
            [app1.id]: this.seedDocumentsFor(app1, (i, total) => i === total - 1 ? 'Revision Required' : i === total - 2 ? 'Under Review' : 'Accepted'),
     ...
     */
    releaseFor(applicationId) {
        const real = this.realPermitResponses()[applicationId];
        if (real)
            return real.release;
        const app = this.applicationById(applicationId);
        if (!app || !this.permitFor(applicationId))
            return null;
     ...
     */
    seedDocumentsFor(app, statusAt) {
        if (app.permitType === 'Business Permit')
            return [];
        ...
        return required.map((doc, i) => {
            ...
            return {
                ...
                remarks: status === 'Revision Required'
    ...
    documentsResolvedFor(applicationId) {
        const app = this.applicationById(applicationId);
        if (!app || app.permitType === 'Business Permit')
            return true;
        const required = requirementsFor(app.permitType).documents.filter((d) => d.required);
        const docs = this.documentsFor(applicationId);
        const unresolved = ['Missing', 'Rejected', 'Revision Required', 'Expired'];
    ...
    requiredDocumentsFor(permitType) {
    ...
    createDraft(input) {
        ...
        if (!actionReferenceIsComplete(input.applicationAction, input.relatedPermitNumber)) {
            throw new Error(`A ${input.applicationAction} application must name the permit it acts on.`);
        }
        appSeq += 1;
        const record = {
            id: nextId('app'),
     ...
     */
    async fileReal(request) {
        try {
            const summary = await firstValueFrom(this.api.fileApplication(request, newIdempotencyKey()));
     ...
     */
    async cancelReal(applicationId, reason) {
        try {
            await firstValueFrom(this.api.cancelApplication(applicationId, reason ? { reason } : {}, newIdempotencyKey()));
            await this.refreshMine();
            return { ok: true };
        }
        catch (error) {
     ...
     */
    async submitPaymentReal(applicationId, body) {
        try {
            const result = await firstValueFrom(this.api.submitPayment(applicationId, body, newIdempotencyKey()));
            await this.refreshMine();
     ...
     */
    reuseDocument(applicationId, requirementId, label, source) {
    ...
    attachDocument(applicationId, requirementId, label, file, fileType, 
    /**
    ...
    submit(applicationId) {
        ...
        this.notifications.push('Application submitted', `Your application ${app.applicationNumber} has been submitted and is queued for review.`, 'application', applicationId);
    }
    updateApplication(id, patch) {
        this.applications.update((list) => list.map((a) => (a.id === id ? { ...a, ...patch } : a)));
    }
    pushTimeline(applicationId, status, remarks) {
        this.timelineByApp.update((map) => ({
            ...map,
            [applicationId]: [...(map[applicationId] ?? []), { status, timestamp: todayIso(), remarks }],
        }));
    }
    issueAssessment(applicationId) {
        const app = this.applicationById(applicationId);
        if (!app)
            return;
        const lineItems = app.applicationAction === 'New'
        ...
        this.notifications.push('Order of Payment issued', `An assessment of ${(total / 100).toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })} is ready for ${app.applicationNumber}.`, 'payment', applicationId);
    }
    submitPayment(applicationId, method, referenceOrProof) {
        const app = this.applicationById(applicationId);
        ...
        this.notifications.push('Payment submitted', `Your payment for ${app.applicationNumber} has been submitted and is awaiting verification.`, 'payment', applicationId);
    }
    /**
     ...
     * transitions.
     */
    advanceForDemo(applicationId) {
        const app = this.applicationById(applicationId);
        if (!app)
            return;
        const idx = LIFECYCLE_SEQUENCE.indexOf(app.lifecycleStatus);
        if (idx < 0 || idx >= LIFECYCLE_SEQUENCE.length - 1)
            return;
        ...
        if (next === 'Payment Under Verification' || next === 'Payment Verified') {
            ...
            if (latest) {
                this.paymentsByApp.update((map) => ({
                    ...map,
                    [applicationId]: (map[applicationId] ?? []).map((p) => p.id === latest.id
            ...
            if (next === 'Payment Verified' && assessment) {
                ...
                if (existing.length > 0) {
                    this.paymentsByApp.update((map) => ({
                        ...map,
                        [applicationId]: (map[applicationId] ?? []).map((tx, i, all) => i === all.length - 1 ? { ...tx, status: 'Verified', verifiedAt } : tx),
                    }));
                }
                else {
                    const tx = {
        ...
        if (next === 'Ready for Release') {
            ...
            this.pushTimeline(applicationId, next, null);
            this.notifications.push('Permit ready for release', `Your permit for ${app.applicationNumber} is ready for release.`, 'permit', applicationId);
            return;
        }
        this.updateApplication(applicationId, { lifecycleStatus: next });
        this.pushTimeline(applicationId, next, null);
