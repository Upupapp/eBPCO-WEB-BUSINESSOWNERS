...
## 5. THE PATCH — 37 files, two groups of change

`docs-sync/2026-08-31-design-system-ahead-of-admin-and-website.patch`

**Group 1 — four documents where the citizen portal is ahead:**
...
**Group 2 — the wrong permit in example copy, 46 occurrences across 33 files:**

| From | To | Count |
|---|---|---|
| `Business Permit Office` | `Office of the Municipal Engineer` | 3 |
| `Business Permit` | `Building Permit` | 43 |

This product issues **19 permit types and not one is a business permit**, and the
issuing office is the Office of the Municipal Engineer per the LGU's own
checklist letterhead. The examples were seeding the wrong permit into
implementations, and it **reached the citizen portal's production code twice** —
a bank account named "eBPCO Business Permits", and generated permits printing
"Business Permit and Licensing Office" as the approving office. Both fixed there.

`Business Name`, `Business Registration`, `Business Owner`, `Business Address`
and similar are **deliberately untouched** — a permit applicant may well have a
business, and blanket-replacing "Business" is the trap this avoids.
...
byte-identical to each other) and **rehearsed end to end**: applied to a pristine
copy of the *website's real* `docs/`, it applies cleanly and leaves all
**158 of 158** files byte-identical to the citizen portal's. Any
`trailing whitespace` warnings are Markdown hard line breaks and are intended.

An earlier draft of this patch was hand-rolled with `diff -u` and **did not
apply** — `git apply` rejected the four paths containing spaces, such as
`04-UX-Standards/06-Error Handling & Recovery.md`, because unquoted spaces break
the `---`/`+++` headers. It is now generated by `git diff` itself, which quotes
them correctly. If you ever hand-roll a patch over a tree with spaces in
filenames, rehearse it before sending.
